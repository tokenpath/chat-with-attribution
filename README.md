...
[`THIRD-PARTY-LICENSES.md`](./THIRD-PARTY-LICENSES.md) ships inside the ZIP.
Minification strips the license headers the bundled dependencies require to
travel with their code, so the notices are reproduced there in full. It is
generated by `npm run licenses` from the production build's own module graph —
so a dependency that is declared but tree-shaken out of the bundle is not
claimed — and CI regenerates and diffs it, so a stale notice fails the build.
